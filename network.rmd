...
mi_network_info <- readRDS("/Users/saraharcos/Desktop/Lauring Lab/timeMI/data/mi_network_info.rds")
mip_noHA<- readRDS("/Users/saraharcos/Desktop/Lauring Lab/timeMI/data/MI_results_091122/h3n2_meaned_mi_noHA.rds") %>%
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
mi <- mi <- readRDS("/Users/saraharcos/Desktop/Lauring Lab/timeMI/data/MI_results_091122/h3n2_meaned_mi.rds") %>%
  select(V1, V2, mip) %>%
  left_join(mi_network_info, by = c("V1" = "index")) %>%
  left_join(mi_network_info, by = c("V2" = "index")) %>%
  select(a = "id.x", b = "id.y", strength = mip) %>%
